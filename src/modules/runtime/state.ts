...
export interface PulseNodeState<TRoot, TValue> {
  ...
  listeners: Set<PulseListener<TValue>>;
  onAccessor?: (listener: PulseListener<TValue>) => () => void;
  readonly path: PulsePath;
  propAccessor?: (key: PropertyKey) => Pulse<unknown> | undefined;
...
const EMPTY_LISTENERS = new Set<PulseListener<unknown>>();
...
export function createRuntime<T>(initialValue: T): PulseRuntime<T> {
  ...
  const rootNode: PulseNodeState<T, T> = {
    ...
    key: null,
    listenerTreeSize: 0,
    listeners: EMPTY_LISTENERS as Set<PulseListener<T>>,
    path: [],
    runtime,
    parent: null,
  };
...
export function subscribeToNode<T>(
  ...
  getMutableListeners(node).add(listener);
