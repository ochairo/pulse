...
function parseComparisonLibraryName(benchmarkName) {
  ...
  if (
    benchmarkName === "Legend-State" ||
    benchmarkName.startsWith("Legend-State ")
  ) {
    return "Legend-State";
  }

  if (benchmarkName === "valtio" || benchmarkName.startsWith("valtio ")) {
    return "valtio";
  }
...
function filterComparisonSections(sections) {
  return sections.map((section) => ({
    ...section,
    cases: section.cases.filter((benchmark) => {
      const library = parseComparisonLibraryName(benchmark.name);
      return library === null || ENABLED_COMPARISON_LIBRARIES.has(library);
    }),
  }));
}
...
export function createComparisonBenchmarkSections() {
  return filterComparisonSections([
    {
